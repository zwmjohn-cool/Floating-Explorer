...
var translations = {
  ...
  "ga": {
    ...
    "collapseAll": "Laghdaigh gach rud",
  ...
  "sq": {
    ...
    "unfocus": "Hiq fokusin",
    "collapseAll": "Palos t\xEB gjitha",
...
var FloatingExplorerPlugin = class extends import_obsidian.Plugin {
  ...
  expandActiveFilePath(leafId) {
    ...
    if (state.focusedFolder) {
      if (!activeFile.path.startsWith(state.focusedFolder + "/") && activeFile.path !== state.focusedFolder) {
        return;
      }
    }
  ...
  buildFolderTree(leafId) {
    ...
    const collapseAllBar = treeContainer.createDiv("collapse-all-bar");
    const collapseIcon = collapseAllBar.createSpan({ cls: "collapse-all-icon" });
    collapseIcon.textContent = "\u25E2";
    const collapseText = collapseAllBar.createSpan({ cls: "collapse-all-text" });
    collapseText.textContent = t("collapseAll", this.locale);
    collapseAllBar.addEventListener("click", () => {
